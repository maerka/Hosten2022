...
Minesweeper::Minesweeper(int width, int height, int mines) {
	...
	for (int i = 0; i < mines; i++) {
		//Plukker tilfeldige tall fra f�rste vektor1. Dette elementet slettes, slik at vektor2 fylles med mines tilfeldige tall
...
void Minesweeper::openTile(int row, int col) {
	...
	if (!isTileMine(row, col)) {
		...
		if (numAdjacentMines(row, col) == 0) { //Hvis ingen miner i n�rheten
...
int Minesweeper::numAdjacentMines(int row, int col) const {
	int nearbyMines = 0;
	/*Tar f�rst de som ikke er i kantene
...
bool Minesweeper::allMinesFound() {
	if (closedTiles - mines == 0) { ////returnerer true hvis antall u�pnede luker = antall miner => alle miner funnet.
		return true;
	}
	else {
		return false;
