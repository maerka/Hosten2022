...
void writeToFile() {
	...
	string newWord;
	while (newWord != "quit") {
		cout << "Skriv inn et ord som skrives til fil. Skriv quit for � avslutte." << endl;
		cin >> newWord;
		if (newWord != "quit") {
			output << newWord << endl;
		}
...
void letterStatistics(string filename) {
	...
	while (!inputFile.eof()) { //G�r gjennom hele dokumentet og f�rer statistikk linje for linje
		...
		for (char testingLetter = 'a'; testingLetter <= 'z'; testingLetter++) {
			int counter = 0;
			for (int i = 0; i < line.size(); i++) { //Leser f�rste linje og teller #a, #b, #c osv.
				if (line[i] == testingLetter || line[i] == char(tolower(testingLetter))) { //SJekker for b�de sm� og store bokstaver
